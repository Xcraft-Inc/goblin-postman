...
const imaps = require('epsitec-imap-simple');
...
Goblin.registerQuest(goblinName, 'openBox', function* (
  ...
  const messages = yield connection.search(finalSearchCriteria, {
    charset,
    ...finalFetchOptions,
  });
  ...
  for (const item of messages) {
    ...
    const content = charset
      ? iconvlite.encode(idHeader + all.body, charset)
      : idHeader + all.body;

    mails.push({
      uid: id,
      mail: yield simpleParser(content, next),
    });
  }
